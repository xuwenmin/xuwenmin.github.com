...
[Underscore](http://underscorejs.org/)是一个提供许多函数编程功能的库，里面包含了你期待(在`Prototype.js`和`Ruby`中)的许多功能。但是没有扩展任何内置的Javascript对象，也就是说它没有扩展任何内置对象的原型。它被定位为`jQuery`和`Backbone.js`的基础层

---

源码注释转之网上他人之备注，特收藏以后方便阅读.
