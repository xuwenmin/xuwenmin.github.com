...
`react`首先它是一个`ui`框架,用来创建独立的前端UI组件用的,类似于`angularjs`里的`指令`以及`polymer`里的`web组件`,为什么说它能够提高`backbone`视图功能,答案就在于它的`独立性`以及`数据同步`功能,类似于`ng`里的数据双向绑定,`react`基于`state`也提供了类似的数据跟视图的同步功能.
